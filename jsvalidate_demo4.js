...
var js_options = {
	...
	errorLocation: "afterEnd", // only accepts beforeBegin, afterEnd, or none (either before or after the input element)
	...
	highlightColor: '#B7E7FF', //default color should be:  #FFFF99
	endColor: '#EEEEEE', //this is what you generally want to set to the background color behind the form elements.
...
function attachSubmit(form_ref){
	var form_attach;
	var parent_ref;
	if(form_ref.getAttribute('name') || form_ref.getAttribute('id')){
		if(document.forms[form_ref.getAttribute('name')]){
			form_attach = document.forms[form_ref.getAttribute('name')];
			form_ref = form_ref.getAttribute('name');
		} else {
			form_attach = $(form_ref.getAttribute('id'));
			form_ref = form_ref.getAttribute('id');
		}
	} else {
...
function attachValidation(fieldref,form_name){	
	...
	if(useBlur){
		...
		if((the_field.tagName == "INPUT" && (field_type == "text" || field_type == "password")) || the_field.tagName == "TEXTAREA"){
			if(window.addEventListener){ // Mozilla, Netscape, Firefox
				the_field.onblur = function(){ blurAction(the_field,form_name); };
			} else { // IE
				the_field.attachEvent('onblur', function(){ blurAction(the_field,form_name); });
			}
		}
		if(the_field.tagName == "SELECT"){
			if(window.addEventListener){ // Mozilla, Netscape, Firefox
				the_field.onchange = function(){ blurAction(the_field,form_name); };
				the_field.onblur = function(){ blurAction(the_field,form_name); };
			} else { // IE
				the_field.attachEvent('onblur', function(){ blurAction(the_field,form_name); });
				the_field.attachEvent('onchange', function(){ blurAction(the_field,form_name); });
			}
		}
		if(the_field.tagName == "INPUT" && field_type == "checkbox"){
			if(window.addEventListener){ // Mozilla, Netscape, Firefox
				the_field.onclick = function(){ blurAction(the_field,form_name); };
			} else { // IE
				the_field.attachEvent('onclick', function(){ blurAction(the_field,form_name); });
			}
		}
		if(the_field.tagName == "INPUT" && field_type == "checkbox"){
			if(window.addEventListener){ // Mozilla, Netscape, Firefox
		...
		if(typeof the_field.nodeType == "undefined"){
			for(a=0; a < the_field.length; a++){
				...
				if(window.addEventListener){ // Mozilla, Netscape, Firefox
					document.forms[form_name].elements[radio_name][a].onclick = function(){ blurAction(radio_name,form_name); };
				} else { // IE
					document.forms[form_name].elements[radio_name][a].attachEvent('onclick', function(){ blurAction(radio_name,form_name); });
				}
			}
		}
	}
