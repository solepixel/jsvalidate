...
var js_options = {
	...
	ajaxURL: '/ajaxtest.php' // if not blank, validator will attempt to check value against this URL; if return value is not blank, error container will be populated with return value
}
...
function onlyJSV(arr){
	...
	for (var i=0; i<arr.length; i++){
		...
		for(items in custom_validators){
			if(arr[i] == custom_validators[items]['className']){
				result[result.length] = arr[i];
			}
		}
		for(items in js_validators){
			if(arr[i] == js_validators[items]['className']){
				result[result.length] = arr[i];
			}
		}
	}
...
function attachSubmit(form_ref){
	...
	if(window.addEventListener){ // Mozilla, Netscape, Firefox
		form_attach.onreset = function(){ return resetAction(form_ref); };
	} else { // IE
		form_attach.attachEvent('onreset', function(){ return resetAction(form_ref); });
	}
	...
	if(getElementsByClassName(js_options['resetClass']).length > 0){
		var buttons = getElementsByClassName(js_options['resetClass']);
		for(r=0; r < buttons.length; r++){
			parent_ref = getParentForm(buttons[r]);
			if(parent_ref == form_ref){
				Event.observe(buttons[r], 'click', function(){ linkedReset = true; document.forms[form_ref].reset(); return false; });
			}
		}
	}
}
...
function attachValidation(fieldref,form_name){	
	...
	if(classes){
		...
		for(c=0; c < classList.length; c++){
			for(items in js_validators){
				if(classList[c] == js_validators[items]['className']){
			...
			for(items in custom_validators){
				if(classList[c] == custom_validators[items]['className']){
					if(validation_message == ""){
	...
	if(validation_message != "" && validation_message != null){
		...
		var field_name = js_options['errorIDPrefix'] + "_" + form_name + "_" + fieldref;
...
function blurAction(field_reference,form_ref){
	...
	if(!isRadio){
		isDisabled = field_reference.disabled;
	}
	if(classes && !isDisabled){
		...
		for(c=0; c < classList.length; c++){
			field_name = js_options['errorIDPrefix'] + "_" + form_name + "_" + the_field;
			field_name = field_name.trim();
			
			for(items in custom_validators){
				if(classList[c] == custom_validators[items]['className'] && field_reference.value != ""){
			...
			if(classList[c] == js_validators['required']['className']){
				isRequired = true;
				if(typeof field_reference.nodeType == "undefined"){
			...
			if(classList[c] == js_validators['notfirst']['className']){
				if(field_reference.selectedIndex == 0 && field_reference.tagName == "SELECT"){
					bad_field = true;
				}
			}
			
			if(classList[c] == js_validators['ajax']['className']){
				isAjax = true;
			}
		}
		...
		if(isAjax){
			if(isRequired && field_reference.value == ""){
				...
				replaceHTML = js_validators['required']['defaultMessage'];
				if(field_reference.getAttribute('alt')){
					if(field_reference.getAttribute('alt') != "" && field_reference.getAttribute('alt') != null){
						replaceHTML = field_reference.getAttribute('alt');
					}
				}
				Element.update(field_name, replaceHTML);
			} else {
				if(ajaxURL){
					bad_field = false;
					ajaxArea = field_name;
...
function checkJSVForm(form_obj){
	var isRadio = false;
	form_name = form_obj.cloneNode(false).getAttribute('name');
	if(form_name){
		fields = getFields(form_name,"name");
	} else {
		fields = getFields(form_obj.getAttribute('id'),"id");
	}
	
	for(f=0; f < fields.length; f++){
		if(document.forms[form_name].elements[fields[f]]){
			the_field = document.forms[form_name].elements[fields[f]];
			if(typeof the_field.nodeType == "undefined"){
				el_type = the_field[0].type.toLowerCase();
			} else {
				el_type = the_field.type.toLowerCase();
...
function resetAction(the_form){
	if(js_options['resetConfirm']){
	...
	for(var v=0; v < jsValidator.length; v++){
		...
		if(form_ref == the_form){
			hideFlag(js_options['errorIDPrefix'] + "_" + form_name + "_" + field_ref);
		}
	}
	if(linkedReset){
		linkedReset = false;
